        ...
        <div className='py-4 px-8 flex justify-center border-b border-b-[rgba(84,84,84,0.3)]'>
            <div className='w-full flex justify-between items-center max-w-screen-xl'>
                ...
                <div className='relative'>
                    <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search here..." className='w-[400px] text-sm outline-none border border-[rgba(84,84,84,0.3)] px-4 py-2 rounded-lg' />
                    {(!_.isEmpty(search)) && <div className='bg-white py-4 absolute top-[100%] w-[350px] rounded-lg drop-shadow-lg min-h-[200px]'>
                        {
                            (!_.isEmpty(results) && !loader) && results.map((item) => {
                                return (
                                    <div onClick={() => {
                                        setSearch("");
                                        ...
                                        <p className='text-xs'>{item.gig.rating}</p>
                                    </div>
                                )
                            })
                        }
                        {
                            (loader) && <div className='flex items-center justify-center px-4'><p className='text-center text-sm'>Loading...</p></div>
                        }
                        {
                            (!loader && _.isEmpty(results)) && <div className='flex items-center justify-center px-4'><p className='text-center text-sm'>No Data Found</p></div>
                        }
                    </div>}
                </div>
                <div>
